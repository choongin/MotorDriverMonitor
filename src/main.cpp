...
#endif
  {
    ...
    // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
    {
      ...
      for (int i=0; i<slave_num; i++){
        ImGui::BulletText("Slave %d Pos: %.2f, Vel: %.2f, Torque: %.2f, Mode: %d, Error: %d", i, f*-1, f*-1, f*-1, slave_num, slave_num);
      }
...
//
//
//      ImGui::Begin("Real Time Graph", &show_graph_window);   // Pass a pointer to our bool variable (the window will have a closing button that will clear the bool when clicked)
